...
## 📌 Live Status Log (confirmed from AWS console, updated 20 July 2026)

| Service | Resource | Status |
|---|---|---|
| Lambda | `vestara-admin-approve-project` | Built — IAM/Secrets Manager permission fix in progress |
| Lambda | `get-deals` | Built — powers project/deals listing |
| API Gateway | `vestara-api` | Routes live: `/admin/projects/{id}/approve` (POST), `/deals` (GET, no authorizer yet) |
| CloudFront | `vestara-frontend` | Live distribution, domain `d2ua02znp13fke.cloudfront.net` |
| S3 | `vestara-frontend-2026` | 4 pages uploaded: `index.html`, `admin.html`, `dashboard.html`, `developer.html` |
| Cognito | User pool `35fbhe`, app client `vestara-web` | Configured — SRP + choice-based sign-in, callback URLs set |

**Confirmed built:** Auth foundation, frontend skeleton (4 static pages), API Gateway with 2 routes, deals listing Lambda, admin approve Lambda (in progress).

**Not yet confirmed built:** KYC/identity verification, AI summaries (Bedrock), invest flow/payments, portfolio tracking, developer document upload, QuickSight reporting.

*(Update this table as new resources are confirmed — keeps one source of truth instead of re-deriving status from screenshots each time.)*
